...
class SearchCoordinator: AppCoordinatorProtocol, ObservableObject {
    ...
    func initial() -> some View {
        SearchViewConfigurator().setup(coordinator: self, categories: Category.categoriesDummy)
    }
    ...
    func resolveView(for route: Route) -> some View {
        ...
        case .detail(let data):
            DetailViewConfigurator().setup(coordinator: self, category: data)
        case .addCategory:
            AddCategoryViewwConfigurator().setup(coordinator: self)
